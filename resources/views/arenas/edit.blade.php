...
<li class="breadcrumb-item active">Edit Arena</li>
@endsection

@section('page-content')
<div class="row">
    <div class="col-8">
        <form class="panel" method="POST" action="{{route('arenas.update',$arena->id)}}" autocomplete="off">
            @csrf
            @method('PUT')
            <div class="panel-body">
                <div class="row ">
                    ...
                    <div class="form-group col-md-8 col-xs-12">
                        <label>Name</label>
                        <input type="text" class="form-control" placeholder="Enter Name" autocomplete="off" name="name" value="{{old('name',$arena->name)}}">
                    </div>
                    <div class="form-group col-md-4 col-xs-12">
                        <label>Status</label>
                        <select class="form-control"autocomplete="off" name="status">
                            @forelse(config('defaults.arena-status') as $status)
                                <option value="{{$status}}" {{$status == old('status',$arena->status) ? 'selected' : ''}}>{{ucfirst($status)}}</option>
                            @empty
                            @endforelse
                        </select>
                    </div>            
